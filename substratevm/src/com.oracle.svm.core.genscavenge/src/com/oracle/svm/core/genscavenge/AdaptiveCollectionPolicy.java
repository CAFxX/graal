 ...
 *
 * NOTE: ParallelGC strictly separates the collection of the young and old generations in minor and
 * major collections, while our GC's full (major) collections collect the young and old generations
 * together. In other words, the cost of one major collection here entails the cost of one young
 * collection and the cost of one old collection is set). With ratios and comparisons of minor and
 * major collections costs, this means that we might grow the old generation too eagerly, while we
 * grow the young generation too hesitantly.
 */
abstract class AdaptiveCollectionPolicy extends AbstractCollectionPolicy {
    ...
    private void updateSizeParametersLocked(SizeParameters params, SizeParameters previous) {
        ...
        if (previous == null || totalGcCount() == 0) {
    ...
    public void onCollectionEnd(boolean completeCollection, GCCause cause) { // {major,minor}_collection_end
        ...
        if (completeCollection) {
            ...
            if (youngGcCount() >= ADAPTIVE_SIZE_POLICY_READY_THRESHOLD) {
    ...
    private long oldGcCount() {
        return avgMajorGcCost.getSampleCount();
    }
    ...
    private long totalGcCount() {
        return avgMinorGcCost.getSampleCount() + avgMajorGcCost.getSampleCount();
    }

    private void decaySupplementalGrowth(boolean completeCollection) {
        if (completeCollection && oldGcCount() % TENURED_GENERATION_SIZE_SUPPLEMENT_DECAY == 0) {
            oldGenSizeIncrementSupplement = oldGenSizeIncrementSupplement.unsignedShiftRight(1);
        }
        if (youngGcCount() >= ADAPTIVE_SIZE_POLICY_READY_THRESHOLD && youngGcCount() % YOUNG_GENERATION_SIZE_SUPPLEMENT_DECAY == 0) {
